...
static int isBMPFormat(unsigned char *aFileHead)//支持bmp
{
 ...
 */
static int GetPixelDatasFrmBMP(unsigned char *aFileHead, PT_PixelDatas ptPixelDatas)//将bmp信息存放在结构体中
{
	...
	{		
		...
		CovertOneLine(iWidth, iBMPBpp, ptPixelDatas->iBpp, pucSrc, pucDest);//将数据转换成结构体，源文件转目标文件
