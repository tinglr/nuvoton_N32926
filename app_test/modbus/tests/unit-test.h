...
const uint16_t UT_INPUT_BITS_ADDRESS = 0x4;
...
const uint16_t UT_REGISTERS_ADDRESS_SPECIAL = 0x10;
/* The response of the server will contains an invalid TID or slave */
const uint16_t UT_REGISTERS_ADDRESS_INVALID_TID_OR_SLAVE = 0x11;
/* The server will wait for 1 second before replying to test timeout */
const uint16_t UT_REGISTERS_ADDRESS_SLEEP_500_MS = 0x12;
/* The server will wait for 5 ms before sending each byte */
const uint16_t UT_REGISTERS_ADDRESS_BYTE_SLEEP_5_MS = 0x13;
...
const uint16_t UT_INPUT_REGISTERS_ADDRESS = 0x8;
