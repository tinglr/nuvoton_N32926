...
#define mtpgt5663_MAX_X  320
#define mtpgt5663_MAX_Y  240
...
static void mtpgt5663_work_func(struct work_struct *work)
{
	...
	{
	    input_report_abs(ts_dev, ABS_MT_POSITION_X, mtpgt5663_events[0].x);
	    input_report_abs(ts_dev, ABS_MT_POSITION_Y, mtpgt5663_events[0].y);
	    input_report_abs(ts_dev, ABS_MT_TRACKING_ID, mtpgt5663_events[0].id);
	    input_mt_sync(ts_dev);	
	}
				  ...
				  const struct i2c_device_id *id)
{
	...
	/* 2.3 这些事件的范围 */
	input_set_abs_params(ts_dev, ABS_MT_TRACKING_ID, 0, mtpgt5663_MAX_ID, 0, 0);
	input_set_abs_params(ts_dev, ABS_MT_POSITION_X, 0, mtpgt5663_MAX_X, 0, 0);
	input_set_abs_params(ts_dev, ABS_MT_POSITION_Y, 0, mtpgt5663_MAX_Y, 0, 0);	
...
//	printk("mtpgt5663_ft5x06_valid : addr = 0x%x\n", client->addr);

//	/* 进一步判断设备的合法性 */
//	buf[0] = 0xa3; /* chip vendor id */	
	//ret = mtpgt5663_ft5x0x_i2c_rxdata(client, buf, 1);
